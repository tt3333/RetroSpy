...
namespace RetroSpy.Readers
{
    public sealed class LibUsbControllerReader : IControllerReader, IDisposable
    {
        ...
        public LibUsbControllerReader(string controllerId, int vid, int pid, ReadEndpointID eid, int readDataLength, int frameSize, int globalOffset, Func<byte[]?, ControllerStateEventArgs?> packetParser)
        {
            _packetParser = packetParser;

            _libusbMonitor = new LibUSBMonitor(controllerId, vid, pid, eid, readDataLength, frameSize, globalOffset);
