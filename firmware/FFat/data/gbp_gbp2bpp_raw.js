...
function refresh()
{
    getTextFromFile("images.dat", function (text) {
        document.getElementById("data_text").value = text;
    });
    ...
    try {
        ...
        var rawBytes = toByteArray(data.value);
        // console.log(rawBytes);

        var packets = parsePackets(rawBytes);
        // console.log(packets);

        var imageStream = getImageDataStream(packets);
        // console.log(imageStream);

        var decompressed = decompressDataStream(imageStream);
        // console.log(decompressed);

        var withDecodedCommands = decodePrintCommands(decompressed);
        // console.log(withDecodedCommands);

        var withHarmonizedPalette = harmonizePalettes(withDecodedCommands);
        // console.log(withHarmonizedPalette);

        var classic = transformToClassic(withHarmonizedPalette);
        // console.log(classic);

        var joined = classic.map((image) => {
            image.unshift('{"command":"INIT"}');
            image.push('{"command":"PRNT","margin_lower":3}');
            return image.join('\n');
        }).join('\n\n\n');
    ...
    tiles_rawBytes_array
        ...
        .forEach(function (tile_element)
        {
            ...
            else
            {
                ...
                {
                    throw new Error('No image start found. Maybe this line is missing')
                }
            }
        })
